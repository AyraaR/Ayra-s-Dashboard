...
// Obtener días de la semana (lunes=0, viernes=4)
function getCurrentDayIndex() {
    const now = new Date();
    const day = now.getDay(); // 0=domingo, 1=lunes...
    if (day === 0 || day === 6) return -1; // fin de semana
    return day - 1; // 0=lunes, 4=viernes
}

// Calcula horas presenciales SOLO para días FUTUROS
function calculateFlexibleHours(teleworkDays, vacationDays, customHoursMap) {
    const currentDayIdx = getCurrentDayIndex();
    
    // Calcular horas ya comprometidas (días pasados + teletrabajo + vacaciones + personalizados)
    ...
    days.forEach((day, idx) => {
        ...
        // Días pasados (ya trabajados o no se pueden cambiar)
        if (idx < currentDayIdx) {
            if (dayConfig.isVacation) committedTotal += 8;
            else if (dayConfig.isTelework) committedTotal += getFixedTeleworkHours(idx);
            else if (dayConfig.customHours) committedTotal += dayConfig.customHours;
            else committedTotal += 8; // día presencial pasado con 8h por defecto
        }
        // Días futuros: teletrabajo y vacaciones son fijos
    ...
    // Días presenciales FUTUROS (sin horas fijas aún)
    const futurePresencialDays = days.filter((day, idx) => {
        if (idx < currentDayIdx) return false;
        const dayConfig = currentSettings[day.id] || {};
        return !dayConfig.isTelework && !dayConfig.isVacation && !dayConfig.customHours;
    });
    
    if (futurePresencialDays.length === 0) return {};
    
    const remainingHours = 40 - committedTotal;
    // Redondear a 2 decimales para evitar 0.1
    const hoursPerDay = Math.round((remainingHours / futurePresencialDays.length) * 10) / 10;
    
    const result = {};
    futurePresencialDays.forEach(day => {
        result[day.id] = hoursPerDay;
    });
    return result;
}
...
function getDayHours(dayId, dayIndex) {
    ...
    // Días pasados: usar el valor guardado o 8 por defecto
...
function renderTable() {
    ...
    days.forEach(day => {
        ...
        row.innerHTML = `
            <td style="padding: 12px;">
                ...
                ${!isTelework && !isVacation && !isPastDay ? '<br><small style="color: var(--accent);">📅 futuro</small>' : ''}
    ...
    // Event listeners solo para días no pasados
...
// Calcular horas REALES acumuladas en la semana (solo días pasados + hoy hasta ahora)
...
function updateSummary() {
    ...
    // Calcular diferencia para llegar a 40
    const remaining = Math.max(0, 40 - actualAccumulated);
    ...
    if (msgSpan) {
        if (actualAccumulated >= 40) {
            msgSpan.innerHTML = '✅ ¡Has cumplido las 40 horas semanales! 🎉';
            msgSpan.style.color = 'var(--success)';
        } else {
            msgSpan.innerHTML = `📊 Llevas ${actualAccumulated.toFixed(1)} horas. Te quedan ${remaining.toFixed(1)} horas para llegar a 40.`;
...
window.calculateActualAccumulatedHours = calculateActualAccumulatedHours;
