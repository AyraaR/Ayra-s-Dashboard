...
function calculateTotalWeekHours(workSettings) {
    ...
    days.forEach((day, idx) => {
        total += getDayHours(day, idx, workSettings);
    });
    return total;
}
...
function updatePreviews() {
    ...
    if (shoppingPreview) {
        ...
        if (items.length === 0) {
            ...
            shoppingPreview.innerHTML = items.slice(0, 3).map(item => 
                `<li><i class="fas fa-circle"></i> ${escapeHtml(item.name)}${item.completed ? ' ✓' : ''}</li>`
            ).join('');
            if (items.length > 3) shoppingPreview.innerHTML += `<li><i class="fas fa-ellipsis-h"></i> +${items.length - 3} más</li>`;
        }
    }
    ...
    if (booksPreview) {
        ...
        if (books.length === 0) {
            booksPreview.innerHTML = '<li><i class="fas fa-book-open"></i> Añade libros leídos</li>';
        } else {
            booksPreview.innerHTML = books.slice(0, 3).map(book => 
                `<li><i class="fas fa-check-circle"></i> ${escapeHtml(book.title)}${book.author ? ` - ${escapeHtml(book.author)}` : ''}</li>`
            ).join('');
            if (books.length > 3) booksPreview.innerHTML += `<li><i class="fas fa-ellipsis-h"></i> +${books.length - 3} más</li>`;
        }
    }
    ...
    if (seriesPreview) {
        ...
        if (pending.length === 0 && watched.length === 0) {
            seriesPreview.innerHTML = '<li><i class="fas fa-tv"></i> Añade series pendientes o vistas</li>';
        } else {
            let html = '';
            if (pending.length > 0) html += `<li><i class="fas fa-clock"></i> Pendientes: ${pending.length}</li>`;
            if (watched.length > 0) html += `<li><i class="fas fa-check-double"></i> Vistas: ${watched.length}</li>`;
            seriesPreview.innerHTML = html;
        }
    }
}
...
function initDockActive() {
    ...
    dockItems.forEach(item => {
        ...
        if ((page === 'home' && currentPage === 'home.html')) {
