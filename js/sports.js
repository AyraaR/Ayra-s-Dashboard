        ...
        resultsDiv.innerHTML = results.map(ex => {
            ...
            return `
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px; border-bottom: 1px solid var(--glass-border);">
                    <div>
                        <strong style="color: var(--accent);">${escapeHtml(ex.name)}</strong>
...
// ============================================================
// FUNCIÓN PARA ELIMINAR ENTRENAMIENTO - AQUÍ ESTÁ EL CÓDIGO
// ============================================================
...
// ============================================================
// FUNCIÓN RENDER WORKOUT HISTORY - AQUÍ VA EL BOTÓN ELIMINAR
// ============================================================
function renderWorkoutHistory() {
    ...
    container.innerHTML = workouts.slice(0, 30).map((workout, idx) => {
        ...
        return `
            <li style="display: flex; justify-content: space-between; align-items: center; padding: 12px; border-bottom: 1px solid var(--glass-border);">
                <div>
                    ...
                    <strong>${escapeHtml(workout.name)}</strong><br>
                    <small>${workout.muscleName} · ${workout.weight}kg x ${workout.sets} series x ${workout.reps} reps · ${workout.volume}kg vol</small>
                </div>
                <div style="display: flex; align-items: center; gap: 10px;">
                    <small>${new Date(workout.date).toLocaleString()}</small>
                    <!-- BOTÓN ELIMINAR - AQUÍ ESTÁ -->
                    <button onclick="deleteWorkout(${idx})" class="btn-danger" style="padding: 4px 8px;">
